...
class node:
    def __init__(self, layer, next_layer=None):
        ...
        if self.next_layer != None:
            # 当前节点到下一节点的权值 randn为标准差为1的正态分布
            self.w = np.random.randn(self.next_layer, self.layer)
...
class data:
    ...
    def loadData(self, mnist_url):
        ...
        training_inputs = [np.reshape(x, (784, 1))
                           for x in training_data[0]]  # （50000，784，1）
        training_results = [self.vectorized_result(
            y) for y in training_data[1]]  # （50000，10，1)
...
def main():
    m_net = net()
    m_data = data()
    training_inputs, training_results, test_data = m_data.loadData(
        'mnist.pkl.gz')
